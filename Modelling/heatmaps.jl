...
# ╔═╡ 7353c580-62fa-11ee-07f7-e18c55b26f07
using DelimitedFiles, DataFrames, CategoricalArrays, MLJ, Plots,ScientificTypesBase,Random, MLJBase, BetaML, MLJDecisionTreeInterface, StatsBase

# ╔═╡ 6b6c7a82-caca-40bc-8952-15d9d2b9148b
using PyPlot

# ╔═╡ e694b439-c0fc-4d4c-b0f3-c114d223ed60
begin 
...
# ╔═╡ 26f09f86-f83a-402c-a425-e9d8433c3709
begin 
	X1 = idata[:,(1:13)]
	y = idata[:,14]
end;

# ╔═╡ 5a3c128f-11b6-4016-b378-32dc78cd5c80
begin
    Random.seed!(411)
    tr_inds, te_inds = MLJBase.partition(1:MLJ.nrows(X1), 0.7, shuffle=true)
end;

# ╔═╡ 3694defd-ad8e-44b3-8961-f484c230b99d
begin
	X2 = X1[:,[1,4,5,8,10]]
	X3 = X1[:, [2,3,6,7,9,11]]
end;

# ╔═╡ fd90911e-cfc4-4bfa-8b3d-3232c1e7f3ab
DTree = @load DecisionTreeClassifier pkg=BetaML verbosity=0

# ╔═╡ 3c266145-1bc7-46aa-994a-fac5687fb7ec
forest = let
    Random.seed!(411)
    tree = DTree()
    EnsembleModel(tree)
...
# ╔═╡ f3463b96-6021-40ee-a9c0-2969d9bd729a
params(forest) 

# ╔═╡ 9c67dccb-6b5b-4d98-9e46-91cb21403cec
begin
	Random.seed!(1)
	r1 = range(forest, :(model.max_depth), lower=1, upper=5)
	r2 = range(forest, :bagging_fraction, lower=0.1, upper=0.8)
	tm = TunedModel(model=forest, tuning=Grid(resolution=10),
	                resampling=CV(nfolds=3,  shuffle = true), ranges=[r1, r2],
	                measure=f1score)
	m = machine(tm, X1, y)
	MLJ.fit!(m, verbosity = 0);
end;

# ╔═╡ 64c6fdca-f9d0-4d42-9a17-9f3e3361e6e4
begin
	rep = report(m)
	rep.best_history_entry.measurement[1]
end

# ╔═╡ abfe1c26-f781-41ae-b8d5-22de52f49db2
rep.best_history_entry

# ╔═╡ f1fdb2f9-cf20-4f99-ab36-298a05970c9d
begin
	res = rep.plotting
...
# ╔═╡ 3e23753a-e3dd-4063-9975-991a91572d71
begin
	pyplot()
...
# ╔═╡ 52e3dab6-6e81-4501-87bd-bd278c544b0e
forest2 = let
    Random.seed!(411)
    tree2 = DTree()
    EnsembleModel(tree2, bagging_fraction = 0.1)
end;

# ╔═╡ 260b9529-53ee-4e60-b9e0-d2d051db182a
begin
	Random.seed!(1)
	r12 = range(forest, :(model.max_depth), lower=1, upper=5)
	r22 = range(forest2, :(model.max_features), lower=1, upper=13)
	tm2 = TunedModel(model=forest2, tuning=Grid(resolution=10),
	                resampling=CV(nfolds=3, shuffle = true), ranges=[r12, r22],
	                measure=f1score)
	m2 = machine(tm2, X1, y)
	MLJ.fit!(m2,verbosity = 0);
end;

# ╔═╡ 307f23cc-f23e-49c0-96dd-6adb6af52b76
begin
	rep2 = report(m2)
	rep2.best_history_entry.measurement[1]
end

# ╔═╡ 9f0c1a1b-5618-4092-b040-34685d1c817e
rep2.best_history_entry

# ╔═╡ 612226be-228e-4100-8948-f7cc2b2c7301
begin
	res2 = rep2.plotting
	vals_sf2 = res2.parameter_values[:, 1]
	vals_bf2 = res2.parameter_values[:, 2];
end;

# ╔═╡ 164c7eae-2439-4f84-9282-af5d7988f769
begin
	pyplot()
	p2 = figure(figsize=(10,6))
	tricontourf(vals_sf2, vals_bf2, res2.measurements, cmap=get_cmap("hot"))
	xlabel("Max Depth", fontsize=14)
	ylabel("Max Features", fontsize=14)
	colorbar(label="RMS")
	p2
end

# ╔═╡ 1228b05f-cf51-40ec-9edc-149f5db6e105
forest_tuned = let
    Random.seed!(1)
...
# ╔═╡ 26221c9f-f598-4989-9221-47bcf20ccd4c
begin
	fmach = machine(forest_tuned, X1, y, scitype_check_level=0)
	MLJ.fit!(fmach, rows=tr_inds, verbosity = 0)
end;

# ╔═╡ 7dc3f58b-307a-43bb-8171-7bfa26cad7b3
md"""
Fully trained random forest results
"""

# ╔═╡ 45edab8f-b49a-45f0-a420-f03ff0cb5edf
begin 
	...
	perff = evaluate!(fmach, measure=f1score, resampling=CV(nfolds=10, shuffle = true), verbosity=0)
	println("F1 Value = $(mean(perff.per_fold[1]))")
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "f9c18fddc05bd4949c76f47170c2666900943d4e"
